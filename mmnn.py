                ...
                if text.lower() == "เชคค่า" or text.lower() == "set":
                    ...
                    if settings["checkContact"] == True:
                        try:
                            ...
                            maxgie.sendMessage(to, "เกิดข้อผิดพลาดในการสำรวจ")
                ...
                if text.lower() == "ขาว":
                  if msg._from in admin:
                      ...
                      try:
                          ...
                          maxgie.sendMessags(to,"Done. >_<")
                elif text.lower().startswith("รูป "):
                  sep = text.split(" ")
                  text = text.replace(sep[0] + " ","")
                  maxgie = requests.get("https://api.eater.pw/googleimg?search={}".format(text))
                  data=json.loads(maxgie.text)['result']
                  raw = []
                  for maxgie in data:
                    if 'http://' in maxgie["img"]:
                        pass
                    else:
                     if len(raw) >= 10:
                       pass
                     else:
                       raw.append({"imageUrl":"{}".format(str(phu["img"])),"action":{"type":"uri","label":"กดสิ","uri":"line://app/1602687308-GXq4Vvk9?type=image&img={}".format(str(phu["img"]))}})
                  k = len(raw)//10
                  for aa in range(k+1):
                    data = {"type":"template","altText":"รูปเกี่ยวกับ " + text,"template":{"type":"image_carousel","columns":raw[aa*10:(aa+1)*10]}}
                    sendTemplate(to,data)
                elif msg.text.lower() == "":
                  if msg.toType == 0:
...
"uri": "https://line.me/ti/p/~nonbysignal",
                ...
                if text.lower() == "ออน" or text.lower() == "runtime":
                   ...
                   txt = msg.text.split(" ")
                   jmlh = int(txt[2])
                   teks = msg.text.replace("แสปม "+str(txt[1])+" "+str(jmlh)+" ","")
                   tulisan = jmlh * (teks+"\n")
                   if txt[1] == "เปิด":
                       if jmlh <= 100000:
                          for x in range(jmlh):
                   ...
                   txt = msg.text.split(" ")
                   jmlh = int(txt[2])
                   teks = msg.text.replace("เทส "+str(txt[1])+" "+str(jmlh)+" ","")
                   tulisan = jmlh * (teks+"\n")
                   if txt[1] == "1":
                       if jmlh <= 100000:
                          for x in range(jmlh):
...
#=====================================================================                
                ...
                if text.lower() == "ปิดมุดลิ้ง":
                    ...
                    duc1(to, "🌟ปิดมุดลิ้งเรียบร้อย🌟")
                        ...
                        if settings["checkContact"] == True:
                            if settings["lang"] == "JP":
                            	duc1(to,"🌟เปิดอ่านข้อความคอนแทคภายในห้อง🌟\n\n꧁{★ढेণढेююທ์★}꧂ ", contentMetadata = {'AGENT_ICON': 'http://dl.profile.line-cdn.net/'+line.getContact(lineMID).pictureStatus, 'AGENT_NAME': 'Creator', 'AGENT_LINK': 'https://line.me/ti/p/Rvv5zHBLDO'})
                            	maxgie.unsendMessage(msg_id)
                        else:
                        ...
                        if settings["checkContact"] == False:
                            if settings["lang"] == "JP":
                                duc1(to,"🌟ปิดอ่านข้อความคอนแทคภายในห้อง🌟\n\n꧁{★ढेণढेююທ์★}꧂ ", contentMetadata = {'AGENT_ICON': 'http://dl.profile.line-cdn.net/'+line.getContact(lineMID).pictureStatus, 'AGENT_NAME': 'Creator', 'AGENT_LINK': 'https://line.me/ti/p/Rvv5zHBLDO'})
                                maxgie.unsendMessage(msg_id)
                        else:
...
#==============================================================================#
                ...
                #=====================================================================              
                  ...
                  me = maxgie.getContact(Mid)
                  cover = maxgie.getProfileCoverURL()
                  pp = me.pictureStatus
                  profile = "https://profile.line-scdn.net/" + str(pp)
                  name = "\n" + me.displayName
                  status = "🌟สเตตัสของเรา🌟\n" + maxgie.getProfile().statusMessage
                  d = Mid
                  maxgie.unsendMessage(msg_id)
                  data={"type": "text","text": "ไล่ดำ"}
                  gid = maxgie.getGroupIdsJoined()
                  for i in gid:
                  	sendTemplate(i, data)
                  print ("ไปไกลๆพวกติดดำ")
