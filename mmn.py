                        ...
                        try:
                            if settings["autolike"] == True:
                                ...
                                if settings["autolike"] == True:
                                    ...
                                    if settings["com"] == True:
                                        maxgie.createComment(msg._from, purl[1], sets["commat"])
                                        wait['postId'].append(purl[1])
                                    else:pass
#=====================================================================
#=====================================================================
...
#=====================================================================
                ...
                if text.lower() == "mid" or text.lower() == "ไอดีเรา":
                    ...
                    maxgie.sendMessage(to, "ส่งรูปภาพลงมาได้เลยครับผม")
                elif text.lower() == 'เปิดออก':
                    settings["autoLeave"] = True
                    maxgie.sendMessage(to, "ออกแชทรวมอัตโนมัติ (เปิด) ใช้งาน")
                elif text.lower() == 'ปิดออก':
                    settings["autoLeave"] = False
                    maxgie.sendMessage(to, "ออกแชทรวมอัตโนมัติ (ปิด) ใช้งาน")   
                if text.lower() == "ออน1":
...
"uri": "https://line.me/ti/p/~nonbysignal",
                ...
                if text.lower() == "ออน" or text.lower() == "runtime":
                   ...
                   if msg.toType == 2:
                      ...
                      num = int(resp)
                      try:
                            maxgie.sendMessage(msg.to,"กำลังดำเนินการ...")
                      except:
                         pass
                      ...
                      maxgie.sendMessage(msg.to,"เชิญคอลสำเร็จแล้ว(｀・ω・´)")
                    ...
                    try:
                        arr = []
                        owner = "uca43cd15fb994f5e04c0984b7c1693ef"
...
#==============================================================================#
                                ...
                                for music in a["items"]:
                                    ret_.append({
                                        ...
                                        "hero": {
                                            ...
                                            "action": {
                                                "type": "uri",
                                                "uri": "line://nv/profilePopup/mid=uca43cd15fb994f5e04c0984b7c1693ef"
                                            }
                                        },
                                        "body": {
                                            "type": "box",
                ...
                if text.lower() == 'ยกเชิญ':
                                if msg._from in Mid:                                
                                    if msg.toType == 2:
                                        group = duck.getGroup(receiver)
                                        gMembMids = [contact.mid for contact in group.invitee]
                                        k = len(gMembMids)//30
                                        duck.sendMessage(msg.to,"[ ยกค้างเชิญ จำนวน {} คน] \nรอสักครู่...".format(str(len(gMembMids))))
                                        num=1
                                        for i in range(k+1):
                                            for j in gMembMids[i*30 : (i+1)*30]:
                                                time.sleep(random.uniform(0.5,0.4))
                                                duck.cancelGroupInvitation(msg.to,[j])
                                                print ("[Command] "+str(num)+" => "+str(len(gMembMids))+" cancel members")
                                                num = num+1
                                            duck.sendMessage(receiver,"กุเหนื่อยรอสักครู่🕛เดียวยกต่อ 30 คน\n ใช่แต่กุ")
                                            time.sleep(random.uniform(15,10))
                                        duck.sendMessage(receiver,"[ ยกค้างเชิญ จำนวน {} คน เรียบร้อยแล้ว👏]".format(str(len(gMembMids))))
                                        time.sleep(random.uniform(0.95,1))
                                        duck.sendMessage(receiver, None, contentMetadata={"STKID": "64104356","STKPKGID": "4108428","STKVER": "1" }, contentType=7)
                                        gname = duck.getGroup(receiver).name
                                        duck.sendMessage(Notify,"[ ยกค้างเชิญ >> "+gname+"  <<] \n จำนวน {} คน เรียบร้อยแล้ว👏\n ใช่กุจังตังไม่ให้สัสส".format(str(len(gMembMids))))
                                        time.sleep(random.uniform(0.95,1))
                                        duck.leaveGroup(receiver)
                               								
                                    duck.sendMessage(receiver,"[ไม่มีค้างเชิญ แล้วนะ😁]")
                                    duck.sendMessage(receiver, None, contentMetadata={"STKID": "64104356","STKPKGID": "4108428","STKVER": "1" }, contentType=7)
                                    duck.leaveGroup(receiver)                    
                    ...
                    try:
                        ...
                        mes = 100
                        M = duck.getRecentMessagesV2(to, 100)
                        MId = []
                        for ind,i in enumerate(M):
                            if ind == 0:
                                pass
                            else:
                                if i._from == duck.profile.mid:
                                    MId.append(i.id)
                                    if len(MId) == mes:
                                        break
                        def unsMes(id):
                            duck.unsendMessage(id)
                        for i in MId:
...
#==============================================================================#
                 ...
                 #   profile = "https://profile.line-scdn.net/" + str(pp)
                  ...
                  #  
                    maxgie.sendMessage(to, "{ Group ID }\n" + gid.id)
...
#=====================================================================
                ...
                if sets["changePictureProfile"] == True:
                    ...
                    maxgie.updateProfilePicture(path)
                    maxgie.sendMessage(to, "ทำการแปลงโฉมเสร็จเรียบร้อย")
        ...
        if op.type == 25 or op.type == 26:
            ...
            if msg.contentType == 0:
                if text is None:
                    return
                if text.lower() == ".":
                    maxgie.sendMessage(to,"[ ꧁{★ढेণढेююທ์★}꧂ ]\nสนใจติดต่อลิ้งด้านล่าง :\nline://ti/p/~꧁{★ढेণढेююທ์★}꧂")
#========================================================================
...
"header": {"backgroundColor": "#EE1289", "separator": True, "separatorColor": "#EE1289"},
"body": {"backgroundColor": "#000000", "separator": True, "separatorColor": "#000000"},
"footer": {"backgroundColor": "#EE1289", "separator": True, "separatorColor": "#EE1289"}
},
"type": "bubble",
"body": {
"contents": [
                    ...
                    if receiver in temp_flood:
                      if temp_flood[receiver]["expire"] == True:
                        ...
                        return
                         ...
                         if temp_flood[receiver]["flood"] >= 40:
                            temp_flood[receiver]["flood"] = 0
                            temp_flood[receiver]["expire"] = True
                            maxgie.sendMessage(to,"มีคนส่งข้อความเกิน40ระบบขอออกอัติโนมัติ")
